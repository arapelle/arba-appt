...
std::filesystem::path spdlogging_impl::make_log_dirpath(const core::program_args& args)
{
    return std::filesystem::temp_directory_path() / make_logger_name(args) / "log"
           / stdx::format_Ymd_HMS_mcs_as_filename();
}

std::string spdlogging_impl::make_logger_name(const core::program_args& args)
{
