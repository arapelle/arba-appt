...
class spdlogging<ApplicationBase> : public ApplicationBase
{
private:
...
public:
    ...
    using rebind_t = spdlogging<ApplicationBase, OtherType>;

    using base_::base_;
};

template <class ApplicationBase, class SelfType>
class spdlogging : public spdlogging<typename ApplicationBase::template rebind_t<SelfType>>, private_::spdlogging_impl
{
private:
    using base_ = spdlogging<typename ApplicationBase::template rebind_t<SelfType>>;

public:
    explicit spdlogging(const core::program_args& args = core::program_args());
    ~spdlogging();
...
template <class ApplicationBase, class SelfType>
spdlogging<ApplicationBase, SelfType>::spdlogging(const core::program_args& args)
...
template <class ApplicationBase, class SelfType>
spdlogging<ApplicationBase, SelfType>::~spdlogging()
{
    if (logger_)
    {
...
template <class ApplicationBase, class SelfType>
std::filesystem::path spdlogging<ApplicationBase, SelfType>::make_log_dirpath() const
{
    return spdlogging_impl::make_log_dirpath(this->args());
}

template <class ApplicationBase, class SelfType>
std::filesystem::path spdlogging<ApplicationBase, SelfType>::make_log_filename() const
{
    return this->self().make_logger_name() + ".log";
}

template <class ApplicationBase, class SelfType>
std::shared_ptr<spdlog::logger> spdlogging<ApplicationBase, SelfType>::make_logger() const
{
    ...
    spdlogging_impl::initialize_logger(logger);
    return logger;
}

template <class ApplicationBase, class SelfType>
std::string spdlogging<ApplicationBase, SelfType>::make_logger_name() const
{
    return spdlogging_impl::make_logger_name(this->args());
}

template <class ApplicationBase, class SelfType>
std::vector<spdlog::sink_ptr> spdlogging<ApplicationBase, SelfType>::make_sink_list() const
{
...
template <class ApplicationBase, class SelfType>
void spdlogging<ApplicationBase, SelfType>::destroy_logger(std::shared_ptr<spdlog::logger> app_logger) const
{
    spdlogging_impl::destroy_logger(app_logger);
}
