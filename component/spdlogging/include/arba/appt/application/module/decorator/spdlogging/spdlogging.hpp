...
class spdlogging<ModuleBase> : public ModuleBase
{
private:
...
public:
    ...
    using rebind_t = spdlogging<ModuleBase, OtherType>;

    using base_::base_;
    virtual ~spdlogging() override = default;
};
...
template <class ModuleBase, class SelfType>
std::filesystem::path spdlogging<ModuleBase, SelfType>::make_log_dirpath() const
{
    return this->app().log_dir();
}

template <class ModuleBase, class SelfType>
std::filesystem::path spdlogging<ModuleBase, SelfType>::make_log_filename() const
{
    return this->self().make_logger_name() + ".log";
}

template <class ModuleBase, class SelfType>
std::shared_ptr<spdlog::logger> spdlogging<ModuleBase, SelfType>::make_logger() const
{
...
template <class ModuleBase, class SelfType>
std::string spdlogging<ModuleBase, SelfType>::make_logger_name() const
{
    return this->name();
}

template <class ModuleBase, class SelfType>
std::vector<spdlog::sink_ptr> spdlogging<ModuleBase, SelfType>::make_sink_list() const
{
...
template <class ModuleBase, class SelfType>
void spdlogging<ModuleBase, SelfType>::destroy_logger(std::shared_ptr<spdlog::logger> mod_logger) const
{
    spdlogging_impl::destroy_logger(mod_logger);
}
