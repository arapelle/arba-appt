...
struct user_sptr_id_hash
{
    ...
    std::size_t operator()(const key_type& arg) const noexcept { return std::hash<key_type>{}(arg); }
...
template <typename UserType>
    requires std::is_base_of_v<user, UserType> && requires(UserType const* user) {
        { user->name() };
    }
...
public:
    ...
    std::size_t operator()(const name_type& arg) const noexcept { return std::hash<name_type>{}(arg); }
...
{
    std::size_t operator()(const arba::appt::user& arg) const noexcept { return arg.id(); }
};
